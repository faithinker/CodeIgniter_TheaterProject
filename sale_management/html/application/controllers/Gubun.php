...
class Gubun extends CI_Controller {       // gubun클래스 선언
    ...
    public function lists(){
        ...
        $data["text1"]=$text1;
        $data["list"]=$this->gubun_m->getlist($text1,$start,$limit);
    ...
    public function del(){
        ...
        redirect("/~sale8/gubun/lists" . $text1 .$page);             // 목록화면으로 돌아가기
    }
    public function add(){
        $uri_array=$this->uri->uri_to_assoc(3);
        $text1 = array_key_exists("text1",$uri_array) ? "/text1/" . urldecode($uri_array["text1"]) : "" ;
        ...
        if($this->form_validation->run()==FALSE){      //추가버튼 클릭시 
            $this->load->view("main_header");          // 상단출력(메뉴)
            $this->load->view("gubun_add");           // gubun_list에 자료전달
            $this->load->view("main_footer");  
        }
        else{
            $data = array(
            ...
            redirect("/~sale8/gubun/lists" .$text1 .$page); 
        }
    }
    public function edit(){
        ...
        if(!$_POST){      //수정버튼 클릭시 
            ...
            $result = $this->gubun_m->updaterow($data, $no); //185p, 원래는 $data, $no 넣는거였음
            redirect("/~sale8/gubun/lists" .$text1 .$page); 
        }
    }
}
