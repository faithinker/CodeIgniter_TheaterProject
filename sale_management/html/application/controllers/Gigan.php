...
class Gigan extends CI_Controller {       // gigan클래스 선언
    ...
    public function view(){
        ...
        //$data["text3"]=$text3;
        $data["page"]=$page;
        $data["row"]=$this->gigan_m->getrow($no);
        
        $this->load->view("main_header");
        //$this->load->view("gigan_title");                  
        $this->load->view("gigan_view",$data);
    ...
    public function lists(){
        ...
        $data["list"]=$this->gigan_m->getlist($text1,$text2,$text3,$start,$limit);
    ...
    $sheet->getActiveSheet()->getStyle("A1")->getFont()->setSize(8);
    ...
    $sheet->getActiveSheet()->getStyle("A2:G2")->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("FFCCCCCC");
    ...
    foreach($list as $row){
        $sheet->setActiveSheetIndex(0)
            ->setCellValue("A$i",$row->writeday8)
            ->setCellValue("B$i",$row->product_name8)
            ->setCellValue("C$i",$row->price8 ? $row->price8 : "")
            ->setCellValue("D$i",$row->numi8 ? $row->numi8 : "")
            ->setCellValue("E$i",$row->numo8 ? $row->numo8 : "")
            ->setCellValue("F$i",$row->prices8 ? $row->prices8 : "")
            ->setCellValue("G$i",$row->bigo8);
        $i++;
    }
