...
<script>
	function find_text(){ //$text1은 상품명이 아니라 캘린더이다.
		form1.action="/~sale8/jangbui/lists/text1/" + form1.text1.value+"/page";
		form1.submit();
	}
...
<form name="form1" action="" method="post">
	<div class="row">
		...
		?>
			<a href="/~sale8/jangbui/add<?=$tmp; ?>" class="btn btn-sm mycolor1">추가</a>
		</div>
	</div>
</form>
...
?>   
		<tr>
						<td><?=$no; ?></td>
						<td><?=$row->writeday8; ?></td>
						<td><a href="/~sale8/jangbui/view/no/<?=$no ?><?=$tmp; ?>"><?=$row->product_name8; ?></a></td>
            <td><?=number_format($row->price8); ?></td>
						<td><?=number_format($row->numi8); ?></td>
